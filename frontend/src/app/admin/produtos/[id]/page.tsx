...
export default function ProdutoFormPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    vendidos_tamanhos: {} as Record<string, number>,
  ...
  const carregarProduto = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const produto: any = response.data;
        ...
        setFormData({
          ...
          estoque_variantes: produto.estoque_variantes || {},
          estoque_cores: produto.estoque_cores || {},
          estoque_tamanhos: produto.estoque_tamanhos || {},
          vendidos_tamanhos: produto.vendidos_tamanhos || {},
      ...
      await showError('Erro ao carregar produto');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className={styles.formGroup}>
              ...
              <select
                ...
                onChange={(e) => setFormData({
                  ...formData,
                  tipo_produto_id: e.target.value ? parseInt(e.target.value) : null
                })}
              >
            ...
            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const precoSeguro = parseNumberOrFallback(e.target.value);

                    if (usarMesmoValorPagamentos) {
                      setFormData({
                        ...formData,
                        preco: precoSeguro,
                        preco_credito: precoSeguro,
                        preco_debito: precoSeguro,
                        preco_boleto: precoSeguro,
                      });
                      return;
                    }

                    setFormData({ ...formData, preco: precoSeguro });
                  }}
                  required
                  placeholder="0.00"
                />
              ...
              {formData.tamanhos_disponiveis.length > 0 && formData.cores_disponiveis.length === 0 && (
                <div className={styles.sizeStockGrid}>
                  {formData.tamanhos_disponiveis.map((size) => {
                    const sizeKey = normalizeSize(size);
                    const vendidos = Number(formData.vendidos_tamanhos[sizeKey] || 0);
